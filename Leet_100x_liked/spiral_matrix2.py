...
#------------------------------
#Approach:
Reuse the inwards_spiral function from Spiral Matrix I to generate indices in spiral order.
Zip through indices and counter, assigning count to the corresponding index of the matrix.
"""
    ...
    def generateMatrix(self, n: int) -> list[list[int]]:
        def inwards_spiral(m: int, n: int) -> Iterator[Tuple[int, int]]:
            ...
            for k in range((min(m, n) + 1) // 2):
                ...
                yield from ((i2, j) for j in range(j2, j1, -1)) if i1 != i2 else (
                    (i2, j2),
                )  # Right to Left
                yield from ((i, j1) for i in range(i2, i1, -1)) if j1 != j2 else (
                    (i2, j1),
                )  # Bottom to Top
